...
M6 begins with a versioned multi-repository change-set contract. It records repository-scoped objectives, dependency and compatibility edges, and deterministic dependency-first layers. Every participating repository pair must mutually opt in through enrollment policy. Migration 26 binds the exact plan to one blocked ledger run/job and immutable policy snapshot per repository. Migration 27 records one principal-scoped, all-members authorization after rechecking current mutual consent and pristine member lineage. Migration 28 can then snapshot current execution/gate policy and derive an immutable initial dependency schedule. These boundaries grant no workspace, model, branch, publication, rollout, or merge authority.
