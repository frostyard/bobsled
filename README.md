...
M8 begins by moving repository enrollment out of runtime source configuration. Migration 47 imports the three reviewed declarations once into a durable, versioned SQLite registry with append-only actor/reason evidence, optimistic concurrency, idempotency, immutable GitHub repository identity, and digest-verified policy reads. The Access surface performs a separate bounded metadata-only drift check and can discover installed Frostyard repositories, import a reviewed `.bobsled/repository.json`, enroll it, disable future work, or re-enable it from freshly verified GitHub identity and policy. GitHub supplies identity; the repository supplies policy; the browser cannot submit either. See [repository enrollment](./docs/repository-enrollment.md).

Migration 48 retains each explicit repository drift check against the exact enrollment version and policy digest. Access page reads are side-effect-free; **Check repository drift** is the only metadata-read/write action. The resulting projection also identifies open, non-archived runs that still carry an older immutable policy snapshot, without changing those runs.
