...
M7 begins with a durable conversational-intake envelope. Migration 41 stores one explicitly selected enrolled repository, a bounded manual or GitHub-issue seed, a live schema-validated brief, and an immutable sequence of principal-owned operator/assistant turns. Creation and turn submission are idempotent, turn updates use optimistic concurrency, repository identity cannot drift, and cancellation is terminal. This foundation is model-free and grants no run admission, provider call, GitHub mutation, or repository research authority.
