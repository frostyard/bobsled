...
test('projects fleet workload, immutable plan quotas, and retention extent without authority', () => {
	...
	try {
		db.exec(`
			...
			CREATE TABLE organization_capacity_claims (id TEXT PRIMARY KEY,status TEXT NOT NULL,openai_codex_slots INTEGER NOT NULL,github_copilot_slots INTEGER NOT NULL,would_exceed_policy INTEGER NOT NULL,expires_at TEXT NOT NULL);
			...
			INSERT INTO organization_capacity_claims VALUES ('claim','active',1,0,1,'2026-09-04T13:00:00.000Z');
			INSERT INTO organization_capacity_claims VALUES ('ambiguous','ambiguous',1,0,0,'2026-09-04T11:00:00.000Z');
