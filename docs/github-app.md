...
The issue-action outbox leaves clix at `readOnly: true` and `writeGitHub: false`. Bobsled's own repository is separately enrolled for issue metadata writes while worker execution, code publication, and merge authority remain disabled. Label execution may change only Bobsled's route-label vocabulary; comment execution uses a durable marker to reconcile retries without duplicate public comments.
