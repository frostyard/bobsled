...
The issue-action outbox leaves clix at `readOnly: true` and `writeGitHub: false`. Bobsled's own repository is separately enrolled for issue metadata writes while worker execution, code publication, and merge authority remain disabled. The public website is separately enrolled for bounded code work and draft-only publication. Label execution may change only Bobsled's route-label vocabulary; comment execution uses a durable marker to reconcile retries without duplicate public comments.

The M4 publication outbox follows the same policy-first rule. Its write token is narrowed to the enrolled repository and the `contents:write` plus `pull_requests:write` profile; lifecycle reconciliation and check polling use separate `pull_requests:read` and `checks:read` profiles. Publication accepts no arbitrary repository, branch, ref update, commit, or PR mode from a model. It binds the approved patch digest and base commit, creates deterministic Git objects, refuses force-push, creates drafts only, and verifies a recovered or refreshed PR still matches its number, URL, branch, commit, base, and durable marker. Required check names are snapshotted at admission and combined with any later policy tightening. There is no merge endpoint. clix keeps publication disabled; `frostyard/frostyard-org` is the first policy-enabled draft-publication acceptance target.
