...
Scope compatibility, patch authorization, integration readiness, workspace assembly, and deterministic three-way recovery do not authorize a model call. Every deterministic projection reports `executionAuthorized: false` or `workerAuthorized: false`; those stages add no worker fan-out, agent-assisted conflict resolution, retry, token spend, concurrent execution, or GitHub capability. The separate migration 17 lease is the first conflict-agent model-call authorization boundary, while migration 18 proves its workspace input before claim. Neither slice has a runner or can dispatch an agent.
