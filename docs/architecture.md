...
This integration path is not yet projected into the main run ledger or operator UI. Repository preparation, final patch revalidation after gates, and a separate conflict-resolution path must exist before the broader M5 execution item is complete.
...
Integration gates declared with `mutatesWorkspace: true` also fail closed. Supporting them safely requires recomputing and re-authorizing the final patch after gate execution; until that later boundary exists, running such a command would invalidate the worker's trusted postcondition evidence. Gate results are durable in the control-plane database but are not yet projected into the main run ledger or operator board.
