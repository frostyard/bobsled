...
The first live acceptance run published frostyard-org PR #6, observed its required Cloudflare check, and handed the draft to a human who merged it. Current publication state intentionally stops at `ready_for_human`: Bobsled does not yet reconcile a later external close or merge into its ledger. The same exercise proved that publication fails closed when the remote default branch advances after approval. A future recovery path may reapply the exact approved patch to a fresh base and rerun trusted validation without another implementation-worker call, but must preserve immutable old-base evidence and block on conflicts or unverifiable context drift.
