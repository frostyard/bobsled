...
The first live acceptance run published frostyard-org PR #6, observed its required Cloudflare check, and handed the draft to a human who merged it. Migration 21 adds explicit authenticated lifecycle reconciliation: merged and closed PRs become durable History state, while an unmerged closed PR remains refreshable so a later reopen returns to check reconciliation. The same exercise proved that publication fails closed when the remote default branch advances after approval. A future recovery path may reapply the exact approved patch to a fresh base and rerun trusted validation without another implementation-worker call, but must preserve immutable old-base evidence and block on conflicts or unverifiable context drift.
