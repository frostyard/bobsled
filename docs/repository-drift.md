...
This check does not enroll, disable, rename, or otherwise mutate a repository. It creates no run, workspace, model call, scheduler claim, policy update, or GitHub write. GitHub discovery, authenticated enrollment/disable actions, persisted observation history, and explicit policy-change handling remain later M8 work.
