...
export const MultiRepositoryMemberExecutionReservationSchema = v.object({
	...
	preflightAuthorized: v.literal(true),
...
interface ReservationRow {
	...
	base_commit: string; workspace_path: string; evidence_path: string; reason: string; status: string; created_at: string;
}
...
export class MultiRepositoryMemberExecutionReservationStore {
	...
	#record(row: ReservationRow): MultiRepositoryMemberExecutionReservation {
		...
		const preparation = lease.preparation?.result;
		if (row.status !== 'reserved' || lease.status !== 'prepared' || !preparation
		...
		return v.parse(MultiRepositoryMemberExecutionReservationSchema, {
			...
			status: row.status, preparationResultSha256: row.preparation_result_sha256,
			...
			reason: row.reason, preflightAuthorized: true, modelDispatchAuthorized: false,
