...
<main>
  ...
  <section class="panel ledger" id="conversation-panel">
    ...
    <div class="conversation-workspace">
      <div><div id="conversation-title" class="notice">Choose “Refine in chat” on a task or issue.</div><div id="chat-log" class="chat-log"></div><form id="chat-form"><label>Clarification or correction<textarea id="chat-message" maxlength="20000" placeholder="Add context, answer a question, or correct the live brief"></textarea></label><div class="toolbar"><button id="chat-submit">Revise brief once</button><button type="button" id="chat-finalize">Finalize brief</button><button type="button" id="chat-correct">Start correction</button><button type="button" id="chat-triage">Run independent triage</button><button type="button" id="chat-admit">Admit triaged run</button><button type="button" class="danger" id="chat-cancel">Cancel conversation</button></div></form></div>
...
const chatCorrect = document.querySelector('#chat-correct');
...
function renderConversation(conversation, revision, shouldScroll=true, snapshot, triage, admission) {
  ...
  if (conversation.supersession) { const lineage=document.createElement('div');lineage.className='notice';lineage.textContent='Superseding correction · source snapshot '+conversation.supersession.sourceSnapshotId.slice(0,12)+' remains immutable';liveBrief.prepend(lineage); }
...
chatCorrect.addEventListener('click',async()=>{if(!activeConversation||activeConversation.status!=='finalized'||activeAdmission)return;const reason=window.prompt('Correction reason (the frozen snapshot remains unchanged):','Operator identified a correction that requires a new immutable snapshot.');if(!reason)return;try{const corrected=await json('/api/intake-conversations/'+activeConversation.id+'/corrections',{method:'POST',headers:{'content-type':'application/json','idempotency-key':browserUuid()},body:JSON.stringify({reason})});renderConversation(corrected);document.querySelector('#chat-message').focus();}catch(error){showError(error);}});
